...
<br/>
  ...
  <button type="submit" name="reg_user" value="submit" style="margin:0; padding:0; display:block; width:100%; max-width:200px; height:40px; background-color:ForestGreen; text-align:center; font:bold 18px/40px helvetica, arial, sans-serif; color:#FFF; border:0; -webkit-appearance: none; -webkit-border-radius: 3px;  -moz-border-radius: 3px; border-radius: 3px; cursor:pointer;" name="_eventId_proceed"">Registe</button>
</div>
</div>
</div>
</form>
...
if(isset($_POST['reg_user'])){
    $query = $db->query("SELECT * FROM idusers where username= '$_POST[username]' ");
    $row = $query->fetch_array(MYSQLI_ASSOC);

    $query1= $db->query("SELECT * FROM idusers where email = '$_POST[email]' ");
    ...
    else if(count($row1['email']) != 0){
            echo "<script>
      window.location.href='register.php';
      </script>";
    }
    else{
      $query2 = $db->query("INSERT INTO  idusers (username, email, password) VALUES ('$_POST[username]', '$_POST[email]', '$_POST[password] ') ");
  echo "<script>
